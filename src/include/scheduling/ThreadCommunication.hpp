...
namespace std {
  ...
  private:
	...
	return _M_value.load(std::memory_order_acquire) & ~_S_locked_bit;
      ...
      {
	return _M_value.load(std::memory_order_acquire) & _S_stop_requested_bit;
      }

      void
      _M_add_owner() noexcept
      {
	_M_owners.fetch_add(1, std::memory_order_relaxed);
      }
	...
	if (_M_owners.fetch_sub(1, std::memory_order_acq_rel) == 1)
      ...
      {
	_M_value.fetch_add(_S_ssrc_counter_inc, std::memory_order_relaxed);
      }

      void
      _M_sub_ssrc() noexcept
      {
	_M_value.fetch_sub(_S_ssrc_counter_inc, std::memory_order_release);
      }
	...
	while (!_M_try_lock(__old, std::memory_order_relaxed))
      ...
      _M_unlock() noexcept
      {
	_M_value.fetch_sub(_S_locked_bit, std::memory_order_release);
      }
	...
	return true;
      ...
      _M_try_lock(value_type& __curval,
		  std::memory_order __failure = std::memory_order_acquire) noexcept
      {
	...
	return _M_do_try_lock(__curval, _S_stop_requested_bit,
      ...
      _M_do_try_lock(value_type& __curval, value_type __newbits,
		     std::memory_order __success, std::memory_order __failure) noexcept
      {
